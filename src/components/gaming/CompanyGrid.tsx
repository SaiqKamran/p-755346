...
export const CompanyGrid = () => {
  ...
  return <section className="py-20 px-4 bg-zinc-950">
      <div className="max-w-[1200px] mx-auto">
        <h2 className="text-3xl font-bold text-white mb-8 text-center">Our Students Work At</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
          {companies.map((company, index) => <Card key={index} className="p-4 flex flex-col items-center justify-center min-h-[120px] bg-white/5 backdrop-blur-lg border-white/10 hover:bg-white/10 transition-colors gap-2">
              <img src={company.logo} alt={`${company.name} logo`} className="w-full h-12 object-contain mb-2" />
