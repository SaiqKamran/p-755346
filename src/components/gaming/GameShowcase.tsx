...
export const GameShowcase: React.FC = () => {
  ...
  return <section className="flex flex-col items-center px-4 relative" ref={showcaseRef}>
      ...
      <div className="relative z-10 w-full flex flex-col items-center">
      ...
      }} falloff="gaussian" radius={200} containerRef={showcaseRef} className="text-white text-4xl md:text-[66px] font-extrabold leading-tight tracking-[2.64px] text-center uppercase max-w-[718px] mt-20 md:mt-[137px] max-md:max-w-full mx-auto px-[41px] my-[21px]" />
        <p className="text-white text-base font-normal leading-[30px] tracking-[0.64px] text-center max-w-[718px] mt-6 max-md:max-w-full mx-auto flex flex-wrap justify-center">
          {text.split(" ").map((word, wordIndex) => <motion.span key={wordIndex} className="mr-2 inline-block" initial={{
          opacity: 0,
          y: 20
        }} whileInView={{
          opacity: 1,
          ...
          margin: "-50px"
              ...
              {word.split("").map((letter, letterIndex) => <motion.span key={`${wordIndex}-${letterIndex}`} className="inline-block" initial={{
            opacity: 0,
            y: 10
          }} whileInView={{
            opacity: 1,
        ...
        <Courses title="Our Course Offerings" description="Transform your creative passion into a successful career with our industry-focused animation and VFX courses" />
      </div>
    </section>;
};